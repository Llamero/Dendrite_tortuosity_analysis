...
t = Table.title();
n_rows = Table.size();
two_channel_dir = getDirectory("Select TWO channel input directory");
two_channel_list = getFileList(two_channel_dir);
three_channel_dir = getDirectory("Select THREE channel input directory");
three_channel_list = getFileList(three_channel_dir);
out_dir = getDirectory("Select the OUTPUT directory");
for(row=0; row<n_rows; row++){
	selectWindow(t);
	path = Table.getString("path", row);
	n_channels = Table.get("channels", row);
	file_name = File.getName(path);
	sample_name = replace(file_name, ".oib", "");
	mask_name = "Dendrite mask - " + sample_name + ".tif";
	process_path = "";
	if(n_channels == 2){
		for(i=0; i<two_channel_list.length; i++){
			if(two_channel_list[i] == mask_name){
				process_path = two_channel_dir + mask_name;
				break;
			}
		}
		if(i == two_channel_list.length){
			waitForUser("ERROR: Could not find a matching file for: " + mask_name);
		}
	}
	else if(n_channels == 3){
		for(i=0; i<three_channel_list.length; i++){
			if(three_channel_list[i] == mask_name){
				process_path = three_channel_dir + mask_name;
				break;
			}
		}
		if(i == three_channel_list.length){
			waitForUser("ERROR: Could not find a matching file for: " + mask_name)
		}
	}
	else{
		waitForUser("Error: " + file_name + " has an invalid number of channels - " + n_channels);
	}
	if(lengthOf(process_path) > 0){
		open(process_path);
		image = getTitle();
		print("Processing image: " + image + ".  " + row+1 + " of " + n_rows + " files.");
		...
		//Measure total mask density
		selectWindow("MAX_Dendrite mask");
		run("Select None");
		getStatistics(dummy, mask_density);
		...
		setResult("Mask_Density", nResults-1, mask_density/255);
		updateResults();
		close("*");
	}
}
